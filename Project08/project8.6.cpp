...
int loadEmployeeFile(Employee* myEmployee)
{
    ...
    else // valid file
    {
		...
		while (!(fileRead.eof())) // read to end of file
		{
			...
			// convert strings to correct data type with error handling
...
void processTransactionFile(Employee* myEmployee, int employeeCount)
{
    ...
    else // process transaction file
    {
    	...
    	while (!(fileRead.eof())) // read transaction file
    	{
			...
			if (index >= 0) // found ID in Employee array
			{
...
/*
	...
	input: string, int
	output: int
	process: validates string stoi
	objectives: validate and return ID from string
*/
int verifyID(string sID, int lineCount)
{
	...
	{
		cout << "ERROR: ID could not convert on line " << lineCount << endl;
...
/*
	...
	input: string, int
	output: float
	process: validates string stof
	objectives: validate and return payRate from string
*/
float verifyPayRate(string sPayRate, int lineCount)
{
	...
	{
		cout << "ERROR: Pay rate could not convert on line " << lineCount << endl;
		return -1; // returns invalid value
	}
	
	return payRate;
} // end of verifyPayRate()

/*
	name: verifyDependents
	input: string, int
	output: int
	process: validates string stoi
	objectives: validate and return dependents from string
*/
int verifyDependents(string sDependents, int lineCount)
{
	...
	{
		cout << "ERROR: Dependents could not convert on line " << lineCount << endl;
		return -1; // returns invalid value
	}
	
	return dependents;
} // end of verifyDependents()
...
/*
	...
	output: int
	process: validates string stoi
	objectives: validate and return type from string
*/
int verifyType(string sType, int lineCount)
{
	...
	{
		cout << "ERROR: Type could not convert on line " << lineCount << endl;
		return -1; // returns invalid value
	}
...
/*
	name: verifyHoursWorked
	input: string, int
	output: float
	process: validates string stof
	objectives: validate and return hours worked from string
*/
float verifyHoursWorked(string sHoursWorked, int lineCount)
{
	float hoursWorked;
