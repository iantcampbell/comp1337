...
class Employee
{
	...
	public:
		...
		// setters
		setEmployee(int setID, string setName, float setPayRate, int setDependents, int setType)
		{
			bool validEmployee;
			validEmployee = false;
			
			if (setID > 0 && setName.length() <= 20 && setPayRate >= 0 && setDependents >= 0 && (setType == 0 || setType == 1))
			{
				id = setID;
				strcpy(name, setName);
				payRate = setPayRate;
...
int populateEmployee(Employee myEmployee[])
{
	...
	else
	{
		...
		while (getline(fileRead, line))
		{
			...
			getline(iss, sID, '\t');
			getline(iss, sName, '\t');
			getline(iss, sPayRate, '\t');
			getline(iss, sDependents, '\t');
			getline(iss, sType, '\n');
			...
			cout << "type: " << type << endl;
			cout << "length: " << sName.length() << endl;
			
			cout << endl;
			
			if (validName)
			{
				validEmployee = myEmployee[employeeCounter].setEmployee(id, sName, payRate, dependents, type);
