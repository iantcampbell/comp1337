...
float max(float);
float min(float);
void getFoodAmounts(monkeysFoodStruct &);
...
int main()
{
	...
	monkeysFoodStruct monkeysFood;

	getFoodAmounts(monkeysFood);
...
void getFoodAmounts(monkeysFoodStruct & monkeysFood)
{
	...
	for ( auto monkey : { "first", "second", "third"} )
    {
    	for ( auto day : { "Sunday", "Monday", "Tusday", "Wednesday", "Thursday", "Friday", "Saturday" } )
    	{
			...
			monkeysFood.foodArray[monkeyCounter][dayCounter] = food;
			dayCounter++
		}
		monkeyCounter++
    }
