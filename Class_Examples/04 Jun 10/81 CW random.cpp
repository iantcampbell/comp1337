...
int main()	
{
	...
	ifstream readFile;  // create an object
	int value;
	...
	countNumber = 0;
	sumNumber = 0;
	// display the count, sum of numbers in the file
	
	readFile.open("data.txt");
	
	if (!readFile)
	{
		cout << "\t\tERR Message! File does not exist" << endl;
	}
	else
	{		
		...
		} // end of 	while (readFile >> value)
		
		readFile.close();
		
		if (countNumber == 0)
		{
			cout << "\n\nNo data in the file" << endl;
		}
		else
		{
			cout << "\n\n\tCount: " << countNumber << endl;
			cout << "\tSum: " << sumNumber << endl;
			
			averageNumber = (float) sumNumber / countNumber;
			cout << "\tAverage: " << averageNumber << endl;
		}
		
	} // else end of if (!readFile)
