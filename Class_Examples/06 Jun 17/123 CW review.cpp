...
void conversion(int eInches, int &yards, int &feet, int &inches)
{

	inches = eInches % 36;	// gives 1
	eInches = eInches / 12; // gives 3
	feet = eInches % 3;		// gives 0
	yards = eInches / 3;	// gives 1
}

// Find gross pay
// parameters are hours worked and wage
// will return the gross pay
// more than 40 hours is overtime

float findGrossPay (float hoursWorked, float wage)
{
	float overTime;
...
void overLoading()
{
	cout << "\tI am starting my earnings" << endl;
}

void overLoading(int value)
{
	cout << "\tI am starting my earnings: " << value << endl;
}
