...
int main()	
{
	...
	int value;
	int countNumber; // counter
	int sumNumber; // accumulator
	
	countNumber = 0;  // initialzie a counter
	sumNumber = 0; // initialzie the accumulator
	
	// pre-read
	value = getIntegerData("\tEnter a value. -999 is the termiantor: ");
	
	while (value != -999)
	{
		countNumber = countNumber + 1;
		sumNumber = sumNumber + value;
		// update entry update - read.. last statement 
		value = getIntegerData("\tEnter a value. -999 is the termiantor: ");
	}
	
	cout << "\n\tNumber count: " << countNumber << endl;
	cout << "\n\tNumber sum: " << sumNumber << endl;
