...
int main()	
{
	...
	// display the sum of numbers between 1 and 50 (inclusive) [5,50] or 5<=x<=50
	// set the numbers >> 1 and 50
	// set the accumulator to 0 before the loop starts and then inside the loop increment
	// display after gong thru the loop
	
	// Variable decleration
	int counter;
	int sumNumbers; // accumulator
	
	counter = 1;  // counter for the loop
	sumNumbers = 0;
	...
	while (counter <= 50)
	{
		...
		sumNumbers = sumNumbers + counter; // accumulating the numbers
		
		// last statement is to increment the counter
		counter = counter + 1; 
	} // end of 	while (counter <= 10)

	cout << "\tSum is: " << sumNumbers << endl;
