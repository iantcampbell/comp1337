...
void populateStudent(Student *myStudent, int studentCount)
{
	...
	{
		myStudent -> name = getStringData("\tEnter name: ");
		for (int examIndex = 0; examIndex < 3; examIndex++)
		{
			myStudent -> examScores[examIndex] = getExamScore(examIndex);
		}
		
		cin.ignore();
		
		if (myStudent -> examScores[1] > myStudent -> examScores[3])
		{
			swap(myStudent -> examScores[1], myStudent -> examScores[3]);
		}
		
		if (myStudent -> examScores[1] > myStudent -> examScores[2])
		{
			swap(myStudent -> examScores[1], myStudent -> examScores[2]);
		}
		
		if (myStudent -> examScores[2] > myStudent -> examScores[3])
		{
...
void sort(Student *myStudent)
{
	...
	if (myStudent -> examScores[2] > myStudent -> examScores[3])
	{
		swap(myStudent -> examScores[2], myStudent -> examScores[3]);
	}
}
